...
app.post("/api/discount/validate", (req, res) => {
  try {
    ...
    if (orderTotal < discountCode.minOrder) {
      return res.json({
        success: false,
        message: `Đơn hàng tối thiểu ${discountCode.minOrder.toLocaleString()}đ để sử dụng mã này!`,
      });
    }

    let discountAmount = 0;
    if (discountCode.type === "percentage") {
      discountAmount = (orderTotal * discountCode.discount) / 100;
    } else {
...
app.post("/api/admin/discounts", checkAdminAuth, (req, res) => {
  try {
    const newDiscount = {
      id: Date.now(),
      ...req.body,
